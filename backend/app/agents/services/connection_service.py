...
def _routing_hint(url: str) -> str:
    ...
    if loopback:
        return (
            f". FinCo-Pilot runs in a container, where localhost is the container itself. "
            f"Use {suggestion} when the model server runs on the host machine."
        )
    if private:
        return (
            f". FinCo-Pilot runs in a container, which often can't reach the host's LAN address. "
            f"Use {suggestion} when the model server runs on the host machine."
        )
    return ""
