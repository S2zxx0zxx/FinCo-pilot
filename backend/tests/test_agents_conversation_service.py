...
async def test_list_conversations_filters_by_agent_and_user(session, test_user, test_workspace, test_agent):
    ...
    rows_a = await svc.list_conversations(session, test_workspace.id, test_user.id, agent_id=test_agent.id)
    assert {c.id for c in rows_a} == {a.id}

    # All of test_user's convs.
    rows_all = await svc.list_conversations(session, test_workspace.id, test_user.id)
    assert {a.id, b.id} <= {c.id for c in rows_all}

    # Foreign user → empty.
    foreign = uuid.uuid4()
    assert await svc.list_conversations(session, test_workspace.id, foreign) == []  # foreign user
...
async def test_update_title_if_empty_only_writes_when_blank(session, test_user, test_workspace, test_agent):
    ...
    refreshed = await svc.get_conversation(session, conv.id, test_workspace.id, test_user.id)
...
async def test_update_title_truncates_to_200_chars(session, test_user, test_workspace, test_agent):
    ...
    long_title = "x" * 500
    out = await svc.update_title(session, conv.id, test_workspace.id, test_user.id, long_title)
...
@pytest.mark.asyncio
async def test_update_title_returns_none_on_missing(session, test_user, test_workspace):
    out = await svc.update_title(session, uuid.uuid4(), test_workspace.id, test_user.id, "x")
    assert out is None
...
async def test_update_title_blank_clears_to_none(session, test_user, test_workspace, test_agent):
    ...
    out = await svc.update_title(session, conv.id, test_workspace.id, test_user.id, "   ")
    assert out is not None
    assert out.title is None


@pytest.mark.asyncio
async def test_delete_conversation_returns_false_on_missing(session, test_user, test_workspace):
    assert await svc.delete_conversation(session, uuid.uuid4(), test_workspace.id, test_user.id) is False
...
async def test_delete_conversation_removes_row(session, test_user, test_workspace, test_agent):
    ...
    assert await svc.delete_conversation(session, conv.id, test_workspace.id, test_user.id) is True
    assert await svc.get_conversation(session, conv.id, test_workspace.id, test_user.id) is None