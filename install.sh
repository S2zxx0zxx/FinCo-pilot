...
REPO_URL="https://github.com/S2zxx0zxx/FinCo-pilot.git"
...
setup_repo() {
  ...
  info "Cloning FinCo-Pilot repository..."
...
# ── Health Check ─────────────────────────────────────────────────────────────
wait_for_health() {
  info "Waiting for FinCo-Pilot to be ready (up to ${HEALTH_TIMEOUT}s)..."
  local elapsed=0

  while [ "$elapsed" -lt "$HEALTH_TIMEOUT" ]; do
    if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
      success "FinCo-Pilot is healthy"
      return
    fi
    sleep 3
    elapsed=$((elapsed + 3))
...
main() {
  echo ""
  echo -e "${BOLD}╔══════════════════════════════════════╗${NC}"
  echo -e "${BOLD}║         FinCo-Pilot Installer             ║${NC}"
  echo -e "${BOLD}╚══════════════════════════════════════╝${NC}"
  echo ""
  ...
  echo -e "${GREEN}${BOLD}  FinCo-Pilot is running!${NC}"
  ...
  echo -e "    ${BLUE}$COMPOSE_CMD_DISPLAY -f $COMPOSE_FILE down${NC}       # Stop FinCo-Pilot"
  echo ""
}
