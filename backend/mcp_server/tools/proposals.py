...
In FinCo-Pilot's own runtime these tools NEVER write to the DB. They return a
structured proposal that the agent surfaces to the user. The user confirms
in the UI, which calls the existing FinCo-Pilot write endpoint to do the real
...
unchanged for FinCo-Pilot's own UI.
"""
...
_PROPOSAL_PREFACE = (
    ...
    "the change happens. In FinCo-Pilot's UI an Apply button + diff card render "
...
_APPLY_FIELD = {
    ...
    "description": (
        ...
        "returning a preview. Ignored by FinCo-Pilot's internal runtime."
    ),
}
