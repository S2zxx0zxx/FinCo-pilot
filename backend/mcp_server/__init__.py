"""FinCo-Pilot's built-in MCP (Model Context Protocol) server.

Runs as a separate container (`mcp-server` in docker-compose, gated by the
`agents` profile). Exposes FinCo-Pilot's read APIs as MCP tools over JSON-RPC 2.0.
...
then triggers the existing FinCo-Pilot write endpoint. This keeps MCP safe.
"""
